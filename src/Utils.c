...
/*
// assuming that zTime is null terminated string
struct tm* zonedDateTimeToLocal(char* zTime, int len) {
    ...
    zTime[4] = '\0';
    int year = strtol(tmp, NULL, 10);
    
    tmp += 5;
    zTime[7] = '\0';
    int month = strtol(tmp, NULL, 10);

    tmp += 3;
    zTime[10] = '\0';
    int day = strtol(tmp, NULL, 10);
    
    tmp += 3;
    zTime[13] = '\0';
    int hours = strtol(tmp, NULL, 10);

    tmp += 3;
    zTime[16] = '\0';
    int minutes = strtol(tmp, NULL, 10);

    tmp += 3;
    zTime[19] = '\0';
    int seconds = strtol(tmp, NULL, 10);
    ...
    int zHrs = strtol(hrsStr, NULL, 10) * 60 * 60;
    int zMin = strtol(minStr, NULL, 10) * 60;
    ...
    struct tm utcTime = *gmtime(&now);
    int utcHrs = hereDT.tm_hour;
    int utcMin = hereDT.tm_min;

    int hrsDiff = (hereHrs - utcHrs) * 60 * 60;
    int minDiff = (hereMin - utcMin) * 60;

    struct tm finalTime;
    finalTime.tm_year = year;
    finalTime.tm_mon = month;
    finalTime.tm_mday = day;
    finalTime.tm_hour = hours;
    finalTime.tm_min = minutes;
    finalTime.tm_sec = seconds;
    finalTime.tm_isdst = 1;

    const time_t origTime = _mktime64(&finalTime);
    const time_t resTime = origTime + hrsDiff + minDiff - zHrs - zMin;

    return localtime(resTime);
